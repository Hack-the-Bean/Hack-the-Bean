...
first_upscale = sr.upsample(img_rgb)

# # Convert result to uint8 format
# first = np.clip(first_upscale, 0, 255).astype(np.uint8)

# Resize LiDAR to match satellite image dimensions
lidar_resized = cv2.resize(log_lidar_data, (first_upscale.shape[1], first_upscale.shape[0]))
...
img_fused = (first_upscale.astype(np.float32) * (1 + lidar_float[:, :, np.newaxis])).clip(0, 255).astype(np.uint8)

# Super-resolve image (3-channel required)
result = sr.upsample(img_fused)

# Convert result to uint8 format
result_image = np.clip(first_upscale, 0, 255).astype(np.uint8)
